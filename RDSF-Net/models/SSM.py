...
class SSM(nn.Module):
    ...
    def forward_corev0(self, x: torch.Tensor, to_dtype=False, channel_first=False):
        ...
        # 把横向和竖向拼接在K维度
        # x_diag = torch.rot90(x, 1, [2, 3]).contiguous().view(B, D, -1)  # 对角线
        # x_antidiag = torch.rot90(x, -1, [2, 3]).contiguous().view(B, D, -1)  # 反对角线

        # # 原始的横向和纵向展开
        # x_horiz = x.view(B, D, -1)
        # x_vert = x.transpose(2, 3).contiguous().view(B, D, -1)
        #
        # # 组合所有方向
        # x_all_dir = torch.stack([x_horiz, x_vert, x_diag, x_antidiag], dim=1)
        # xs = torch.cat([x_all_dir, torch.flip(x_all_dir, dims=[-1])], dim=1)
...
class space(nn.Module):
    ...
    def __init__(self, dim):
        ...
        self.conv0 = nn.Sequential(nn.Conv2d(dim, dim, 3, padding=1, bias=False, groups=dim),
                                   nn.ReLU(inplace=True),
                                   nn.Conv2d(dim, dim, 3, padding=1, bias=False, groups=dim),
                                   nn.ReLU(inplace=True),
                                   nn.Conv2d(dim, dim, 1)
                                   )
...
class OSSBlock(nn.Module):
    def __init__(
        ...
        try:
            ...
            _OSSM = SS2DDev if forward_type.startswith("dev") else SSM
        except:
            _OSSM = SSM
